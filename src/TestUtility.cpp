...
void TestUtility::RunTest(const string &test_info,
                          const function<bool(DBProvider *)> sut,
                          DBProvider *db_prov) {
...
void TestUtility::RunTest(const string &test_info,
                          const function<bool(shared_ptr<DBProvider>)> &sut,
                          shared_ptr<DBProvider> db_prov) {
  cout << yellow << "\nRUNNING: " << reset << test_info << endl;
...
void TestUtility::RunAll(
    const vector<pair<const string, function<bool(DBProvider *)>>>
        &provider_tests,
    DBProvider *db_prov) {
  cout << yellow << "\t##########\t"
       << "\tTESTING STARTED\t"
...
void TestUtility::RunAll(
    const vector<pair<const string, function<bool(shared_ptr<DBProvider>)>>>
        &provider_tests,
    const shared_ptr<DBProvider> &db_prov) {
  cout << yellow << "\t##########\t"
       << "\tTESTING STARTED\t"
       << "\t##########" << reset << endl;
...
void TestUtility::RunSimpleTests(
    const vector<pair<const string, function<bool()>>> &simple_tests) {
  cout << yellow << "\t##########\t"
