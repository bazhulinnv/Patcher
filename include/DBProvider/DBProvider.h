...
class DBProvider {
public:
  explicit DBProvider(const string &login_string_pg);

  explicit DBProvider(
      shared_ptr<DBConnection::Connection> already_set_connection);
...
private:
  ...
  Trigger GetTrigger(const ObjectData &data, const string &comment = "",
                     const string &code = "") const;

  Sequence GetSequence(const ObjectData &data, int start_value = 1,
  ...
  void InitializePartitionExpression(Table &table,
