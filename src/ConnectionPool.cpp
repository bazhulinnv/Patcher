...
void ConnectionPool::ResetPoolParameters(const LoginData &params) {
  ...
  conn_pool_log.AddLog(
      PatcherLogger::INFO,
      "Connecting through libpqxx to database with params:\n\t\t" +
  ...
  for (auto &conn : pool_) {
    try {
      ...
      conn_pool_log.AddLog(PatcherLogger::WARNING,
                           "DBConnectionPool exception: Couldn't connect to "
                           "database with:\n\t\t" +
                               params.LoginString_Pqxx());
  ...
  if (actual_quantity == 0) {
    conn_pool_log.AddLog(PatcherLogger::ERROR,
                         "DBConnectionPool exception: ALL CONNECTIONS FAILED.");
    cerr << "DBConnectionPool Pool: ALL CONNECTIONS FAILED.\n Check "
            "GetConnectionFromPool parameters"
         << endl;
...
void ConnectionPool::CreatePool(string &pgpass_string) {
  ...
  for (int i = 0; i < pool_size_; ++i) {
    try {
      ...
      cerr << "CONNECTION POOL: Connection failed, params" << pgpass_string
           << endl;
      cerr << "\t\t" << err.what() << endl;
    }
  }
