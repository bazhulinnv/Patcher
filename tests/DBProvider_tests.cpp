...
bool Test_GetFunctions(DBProvider *db_prov) { return false; }

bool Test_GetFunctionParameters(DBProvider *db_prov) { return false; }

bool Test_GetTriggers(DBProvider *db_prov) { return false; }
...
bool Test_PrintFunctionParameters(DBProvider *db_prov) { return false; }

bool Test_PrintTriggers(DBProvider *db_prov) { return false; }
...
void RunConnectionPool(const shared_ptr<DBConnection::ConnectionPool> &pg_pool,
                       size_t id) {
  ...
  try {
    ...
    pqxx::work work(
        reinterpret_cast<pqxx::connection &>(*conn->GetConnection()));
...
void TestConnectionPool_MultiThreading(string &pg_login) {
  ...
  // number of thread here should be number of thread available in your thread
  // pool waiting for incoming job to execute imagine X threads are waiting to
  // execute task
...
void RunTest_PrintScriptsText(DBProvider *db_prov) {
  cout << cyan << "\nRUNNING TEST: SCRIPT DEFINITIONS:" << reset << endl;

  ObjectData test_view("pg_roles", ObjectType::ViewType, "");
  ObjectData test_sequence("math_blob_id_seq", ObjectType::SequenceType,
                           "public");
  ObjectData test_index("math_blob_1_id_idx", ObjectType::IndexType, "public");
  ObjectData test_function("clear_serie", ObjectType::FunctionType, "io",
                           {"p_serie_path"});
  ObjectData test_trigger("placeholder", ObjectType::TriggerType, "public");
...
int main() {
  ...
  simple_tests.emplace_back(
      "Standard Test : make sure pqxx::GetConnectionFromPool works",
      Test_PqxxConnection);

  simple_tests.emplace_back("Test PatchLogger::Log : write to different logs",
                            Test_Logger);
  /*simpleTests.push_back(make_pair("Test Connection : connect to 'Doors'",
   * Test_CustomConnection));*/

  vector<pair<const string, function<bool(DBProvider *)>>> provider_test;
  vector<pair<const string, function<bool(shared_ptr<DBProvider>)>>>
      provider_tests_shared;

  provider_test.emplace_back(
      "Test printing raw data : printing raw objects data",
      Test_PrintObjectsData);

  provider_test.emplace_back(
      "Test printing data in table : printing objects data wrapped in table",
      Test_PrintObjectsAsTable);

  provider_test.emplace_back("Test printing all tables : printing tables in "
                             "io, public, common, secure",
                             Test_PrintTableList);

  provider_test.emplace_back("Test : check table existence", Test_tableExists);

  provider_test.emplace_back("Test : check SequenceType existence",
                             Test_sequenceExists);

  // TestConnectionPool_MultiThreading(loginData);

  RunTest_PrintScriptsText(db_provider);

  PrintAllFunctionParams(db_provider);

  provider_test.emplace_back("Test functions", Test_GetFunctions);

  provider_test.emplace_back("Test function parameters",
                             Test_GetFunctionParameters);

  provider_test.emplace_back("Test triggers", Test_GetTriggers);

  provider_test.emplace_back("Test printing functions", Test_PrintFunctions);

  provider_test.emplace_back("Test printing function parameters",
                             Test_PrintFunctionParameters);

  provider_test.emplace_back("Test printing triggers", Test_PrintTriggers);
