...
bool DependenciesChecker::getIfAllObjectsExists() { return all_objects_exists; }

std::string DependenciesChecker::getDataForLog() { return data_for_log; }

bool DependenciesChecker::getCheck(DBObjs &objects_parameters,
...
bool DependenciesChecker::check(DBObjs &objectsParameters,
  ...
  for (auto i : objectsParameters) {
    ...
    // generate list of existence each object in format: 0 if object does not
    // exist in current database, 1 if exists
    existence_each_object.emplace_back(does_current_object_exist);

    // generate data for log: object parameters + existence; example public cats
    // table exists
    data_for_log +=
        std::get<0>(i) + " " + std::get<1>(i) + " " + std::get<2>(i) + " ";
    if (!does_current_object_exist) {
