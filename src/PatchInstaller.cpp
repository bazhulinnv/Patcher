...
bool PatchInstaller::checkDependencyList(std::string file_name,
  ...
  if (parser.checkInputCorrect(file_name)) {
    ...
    // the file
    DBObjects objects_parameters = parser.getResultOfParsing(file_name);
    DependenciesChecker checker;

    // result = true if all object exist in current database
    bool result = checker.getCheck(objects_parameters, dbProvider);
...
void passInstallLogsGui(std::string &data_for_error_log, bool install_with_errors) {
